...
[CreateAssetMenu(fileName = "New Mana Decrease", menuName = "Effect/ManaHeal")]
public class ManaHeal : EffectInf
{
    ...
    // Helper method to apply the mana heal effect
    private async void ApplyManaHeal(ApplyEffectEventArgs e)
    {
        if (e.Card.CardOwner == PlayerID.Player1)
        {
            if (ApplyToMyself)
            {
                await effectMethod.P1ManaHeal(e, healAmount, this);
            }
            else
            {
                await effectMethod.P2ManaHeal(e, healAmount, this);
            }
        }
