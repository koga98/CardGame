...
[CreateAssetMenu(fileName = "New Heal Shield Card", menuName = "Effect/ProtectShieldHeal")]
public class ProtectShieldHeal : EffectInf
{
  ...
  {
    // Apply the main effect based on conditions
    if (AreConditionsMet(conditionOnEffects, e))
    {
      await ApplyHealShield(e);
    }
  ...
  // Helper method to apply heal shield effect based on player and target
  private async Task ApplyHealShield(ApplyEffectEventArgs e)
  {
    if (e.Card.CardOwner == PlayerID.Player1)
    {
      if (ApplyToMyself)
      {
        await effectMethod.P1HealShield(e, healAmount, this);
      }
      else
      {
        await effectMethod.P2HealShield(e, healAmount, this);
      }
    }
